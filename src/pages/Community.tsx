...
type ViewState = "main" | "browse" | "detail";
...
const Community = () => {
  ...
  const selectedCategory = categories.find((c) => c.id === selectedProgram);
  ...
  if (view === "browse") {
    return (
      <div className="min-h-screen bg-background">
        ...
        <div style={{ background: "hsl(199 89% 48%)" }}>
        ...
        <div className="px-4 pt-6 grid grid-cols-2 gap-3">
          {categories.map((cat) => (
            <button
              key={cat.id}
              onClick={() => {
                setSelectedProgram(cat.id);
                setView("detail");
              }}
              className="rounded-2xl py-5 px-4 text-center font-bold text-base"
              style={{
  ...
  if (view === "detail" && selectedCategory) {
    return (
      <div className="min-h-screen bg-background">
        {/* Blue header with arc */}
        <div style={{ background: "hsl(199 89% 48%)" }}>
          <div className="relative pt-12 pb-6 flex flex-col items-center px-6">
            <button
